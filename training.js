  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function defaultState(){
      return {
        ...
        simulations: {
          "Séance 1": {
            name: "Séance 1",
            sequences: [{ name: "Séquence 1", objects: [], lastSnapshot: [] }],
            currentSeq: 0
          }
        }
      };
    ...
    function attachUI(){
      ...
      els.newSeq.addEventListener("click", ()=>{
        ...
        if (!name) return;
        sim.sequences.push({ name, objects: [], lastSnapshot: [] });
    ...
    function drawObjects(showHandles = true){
      ...
      seq.objects.forEach(obj => {
        ...
        if (selectedId === obj.id && showHandles){
          ...
          const boxSize = (obj.w||80) * (obj.scale||1);
    ...
    async function drawStateToCanvas(objects, existingCanvas, existingCtx){
    ...
    async function exportPng(){
      const canvas = await drawStateToCanvas(getCurrentSeq().objects);
      canvas.toBlob(blob => {
        if (!blob) return;
    ...
    async function exportVideo(){
      ...
      async function frameStep(){
        ...
        const blend = interpolateStates(startState, endState, t);
        await drawStateToCanvas(blend, canvas, ctx);
        frame++;
        if (frame < frames){
